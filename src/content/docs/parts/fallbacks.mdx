---
title: "Fallbacks"
description: "Handle overflow gracefully with position-try-fallbacks, @position-try rules, and position-try-order."
sidebar:
  order: 3
---
...
In the [previous posts](/posts/getting-started-with-css-anchor-positioning-anchor-name-position-anchor-and-position-area/), we covered how to position elements relative to anchors using `position-area` and the `anchor()` function. But there's a problem we haven't addressed: what happens when your carefully positioned tooltip, dropdown, or popover doesn't fit?
...
{/* Add your Baseline Status component here */}
...
| `flip-start` | Swaps start ↔ end on both axes |
| `flip-x` | Mirrors horizontally (physical) |
| `flip-y` | Mirrors vertically (physical) |

The logical keywords (`flip-block`, `flip-inline`, `flip-start`) adapt to writing modes. The physical keywords (`flip-x`, `flip-y`) always flip in the same direction regardless of writing mode — use these only when you have a specific reason to.
...
.dropdown {
  ...
  position-try-fallbacks: 
    flip-block,                /* Try above */
    flip-inline,               /* Try to the right */
...
- A different anchor element altogether

The `@position-try` at-rule lets you define named fallbacks with their own positioning properties. This works regardless of whether your original positioning uses `position-area`, `anchor()`, or a combination of both.
...
.tooltip {
  ...
  position-try-fallbacks: 
    flip-block,         /* First: try flipping below */
    --compact-below;    /* Last resort: below, but constrained height */
}
```
...
.dropdown-menu {
  ...
  position-try-fallbacks:
    --above-aligned,       /* Flip above if no room below */
    flip-inline,           /* Try other side if still tight */
...
2. **Only triggers on overflow** — If your element fits in its original position, fallbacks are ignored entirely. There's no performance cost for defining fallbacks that aren't needed.
...
[popover] {
  ...
  position-try-fallbacks: 
    flip-block,              /* Try above, same alignment */
    flip-inline,             /* Try below, opposite alignment */
...
- **Screen readers see the same content** regardless of which position is used. This is generally good — the fallback is a visual adaptation, not a content change.
