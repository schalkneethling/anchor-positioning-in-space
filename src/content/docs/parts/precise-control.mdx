...
description: "Go beyond the grid with anchor(), anchor-center, and anchor-size() for pixel-level positioning and sizing."
sidebar:
  order: 2
---
...
.tooltip {
  ...
  inset-inline-start: calc(anchor(end) + 0.5rem); /* To the inline-end, with gap */
...
The guidance from the previous post applies here too:

- **Focus management**: DOM order should make sense for keyboard and screen reader users, regardless of visual positioning.
- **Visibility**: Ensure positioned elements remain visible when focused. Test with keyboard navigation.
- **Semantic relationships**: Visual proximity doesn't create semantic relationships. Use appropriate ARIA attributes when needed.

Anchor positioning handles where things appear. You're still responsible for ensuring the experience makes sense for all users.
