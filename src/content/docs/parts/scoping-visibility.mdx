---
title: "Scoping and Visibility"
description: "Manage anchor relationships at scale with anchor-scope, implicit anchors, and position-visibility."
sidebar:
  order: 4
---

In the previous posts, we covered [the fundamentals](/posts/getting-started-with-css-anchor-positioning-anchor-name-position-anchor-and-position-area/) of anchor positioning, [precise control](/posts/precise-positioning-with-css-anchor-functions/) with `anchor()` and `anchor-size()`, and [fallback strategies](/posts/css-anchor-positioning-fallbacks/) for when things do not fit.
...
This final post covers the three features that answer those questions: [`anchor-scope`](https://developer.mozilla.org/en-US/docs/Web/CSS/anchor-scope), implicit anchor elements, and [`position-visibility`](https://developer.mozilla.org/en-US/docs/Web/CSS/position-visibility).
...
The [`anchor-scope`](https://developer.mozilla.org/en-US/docs/Web/CSS/anchor-scope) property limits where anchor names are visible. Apply it to a common ancestor, and names defined inside that subtree become invisible to elements outside it — and positioned elements inside can only match anchors within the same subtree.
...
```html
<body> {/* containing block for the fixed tooltip */}
...
<div role="tooltip" id="tooltip-1" class="tooltip">
...
- [`anchor-scope` on MDN](https://developer.mozilla.org/en-US/docs/Web/CSS/anchor-scope)
